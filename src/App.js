...
import { Briefcase, TrendingUp, TrendingDown, DollarSign, List, Clock, Search, X, CheckCircle, Bell, ArrowLeft, History, Activity, Check, Power, Trash2, Volume2, BellRing, VolumeX } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    const getInitialData = async () => {
        try {
            ...
            initialLoadComplete.current = true; // Tandai bahwa data awal sudah dimuat
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
        if (!initialLoadComplete.current) return; // Jangan jalankan jika data awal belum ada
